# -*- coding: utf-8 -*-
"""
Created on Fri Nov 12 19:41 2020
...
#print('  (3) Generar efemerides de magnitud.')
print('  (9) Salir.', sep='')
fileMode = 0
while True:
...
# Especificación de fechas de figuras
print('* Ingrese fechas para figuras o deje en blanco para usar valores por defecto.') 
while True:
    ephInitDate = input('  - Fecha inicial (YYYY-MM-DD): ')
    if  (not ephInitDate) | valiDate(ephInitDate):
        break

while True:
    ephEndDate = input('  - Fecha final (YYYY-MM-DD): ')
    if  (not ephEndDate) | valiDate(ephEndDate):
        break    
...
if ephInitDate:
    print('  - Fecha inicial de figuras: ' + ephInitDate)
else:
    print('  - Fecha inicial de figuras por defecto.') 

if ephEndDate:
    print('  - Fecha final de figuras: ' + ephEndDate)
else:
    print('  - Fecha final de figuras por defecto.') 
...
if figs:
    ...
    # Calculo de fechas para gráficos
    if ephInitDate:     # Si se ingresó fecha inicial para figuras
        ephInitJD = strDate2jd(ephInitDate)
    else:               # Fecha inicial de figuras por defecto
        ephInitJD = np.floor(minJD)-4.5

    if ephEndDate:     # Si se ingresó fecha final para figuras
        ephEndJD = strDate2jd(ephEndDate)
    else:               # Fecha final de figuras por defecto
        ephEndJD = np.floor(maxJD)+5.5

    figJD = np.arange(ephInitJD,ephEndJD+1, 1)
